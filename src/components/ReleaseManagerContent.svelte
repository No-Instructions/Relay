<script lang="ts">
	...
	onMount(() => {
		...
		// Debug to check the current plugin version
		console.log("Current plugin.version:", plugin.version);
		console.log("Current plugin.manifest.version:", plugin.manifest.version);
	});
	...
	$: {
		if (githubReleases && githubReleases.length > 0) {
			...
			githubReleases.forEach(async (release) => {
				try {
					const manifest = await plugin.updateManager.fetchReleaseManifest(
						release.tag_name,
					);
					if (manifest) {
	...
	async function viewReleaseManifest(tagName: string) {
		...
		try {
			...
			manifestError.set(
				`Error fetching manifest: ${error.message || "Unknown error"}`,
			);
		} finally {
	...
	// Function to show development version info
	function showDevelopmentVersion() {
	...
	async function installSpecificVersion(tagName: string) {
		try {
			...
			// Check if we're installing a version from an alias
			if ($manifestVersions[tagName] === "stable-alias") {
				manifest = $stableManifest;
			} else if ($manifestVersions[tagName] === "beta-alias") {
				manifest = $betaManifest;
			} else {
				// Regular release tag, fetch its manifest
				manifest = await plugin.updateManager.fetchReleaseManifest(tagName);
			}
			...
			// Don't apply this to alias versions since they don't correspond to Git tags
			if (
				$overwriteVersionOnInstall &&
				$manifestVersions[tagName] !== "stable-alias" &&
				$manifestVersions[tagName] !== "beta-alias"
			) {
				...
				plugin.log(
					`Overwriting manifest version with Git tag: ${normalizedTag}`,
				);
			}
			...
			// Special case for aliases to use the main branch
			const gitTagName =
				$manifestVersions[tagName] === "stable-alias" ||
				$manifestVersions[tagName] === "beta-alias"
					? "main"
			...
			await app.plugins.installPlugin(repoPath, gitTagName, manifest);
...
<div class="modal-content">
	...
	<div class="settings-container">
		...
		<SlimSettingItem name="Use Git tag as version">
			...
			>
				<input type="checkbox" checked={$overwriteVersionOnInstall} />
				<div class="checkbox-toggle"></div>
			</div>
		</SlimSettingItem>
	</div>
