<script lang="ts">
	...
	let canEditName = writable(false);
	$: {
		const currentUserId = plugin.relayManager.user?.id;
		if (!currentUserId) {
			canEditName.set(false);
		} else {
			const isFolderCreator = remoteFolder?.creatorId === currentUserId;
			...
			canEditName.set(isFolderCreator || isRelayOwner || isFolderOwner);
		}
	}

	// Keep the original isOwner for other permissions (users management, etc.)
	let isOwner = writable(false);
	$: {
		const isFolderCreator =
			remoteFolder?.creatorId === plugin.relayManager.user?.id;
		const isRelayOwner = $relayStore?.owner === true;
		isOwner.set(isFolderCreator || isRelayOwner);
	}
...
<Breadcrumbs
	items={[
		...
		{
			type: "remoteFolder",
			remoteFolder: { ...remoteFolder, name: currentDisplayName, private: isPrivate }
		}
	]}
/>
...
{#if $relayStore}
	{#if $relayStore?.owner || $folderStore?.remote?.creatorId === plugin.relayManager.user?.id}
