<script lang="ts">
	...
	function handleManageRelayEvent(event: ManageRelayEvent) {
		console.log("PluginSettings handleManageRelayEvent called with:", event.detail);
		history.push({
			currentRelay,
			sharedFolder,
			remoteFolder,
		...
		remoteFolder = undefined;
		currentComponent = ManageRelay;
		console.log("PluginSettings switched to ManageRelay component with relay:", currentRelay);
	}
	function handleManageSharedFolderEvent(event: ManageSharedFolderEvent) {
		...
		// Use ManageRemoteFolder for remote-only folders, ManageSharedFolder for local folders
		currentComponent = event.detail.folder
			? ManageSharedFolder
			: ManageRemoteFolder;
	}
