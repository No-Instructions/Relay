<script lang="ts">
	...
	$: syncSettings = sharedFolder?.syncSettingsManager;
	let relayStore = derived([relayRoles], ([$relayRoles]) => {
		if (!sharedFolder?.remote) return undefined;
		return $relayRoles.find((role) => role.relay === sharedFolder.remote.relay)
			?.relay;
	});

	let noStorage = derived([relayRoles], ([$relayRoles]) => {
		if (!sharedFolder?.remote) return false;
		return (
			$relayRoles.find((role) => role.relay === sharedFolder.remote.relay)
