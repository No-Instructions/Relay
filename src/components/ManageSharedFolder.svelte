<script lang="ts">
	...
	const syncSettingsStore = writable<SyncFlags>({});

	// Subscribe to changes and update the store
	const unsubscribe = syncSettings.subscribe((value) => {
		syncSettingsStore.set(value);
	});
	...
	{#each settingEntries as [name, category]}
		<SettingItem name={category.name} description={category.description}>
			<div class="setting-item-control">
				<div
					role="checkbox"
					aria-checked={$syncSettingsStore[name]}
					tabindex="0"
					on:keypress={() => handleToggle(name, !$syncSettingsStore[name])}
					class="checkbox-container"
					class:is-enabled={$syncSettingsStore[name]}
					on:click={() => handleToggle(name, !$syncSettingsStore[name])}
				>
					<input
						type="checkbox"
						checked={$syncSettingsStore[name]}
						disabled={isUpdating}
