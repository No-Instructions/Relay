...
export class BackgroundSync {
	...
	async enqueueSync(item: Document): Promise<void> {
		...
		if (this.inProgressSyncs.has(item.guid)) {
			this.debug(`[enqueueSync] Item ${item.guid} already in progress, skipping`);
		...
		// Update the counters for individual document sync
		group.syncs++;
		group.total++;
		...
		// Create a promise that will resolve when the sync completes
	...
	enqueueDownload(item: Document): Promise<void> {
		...
		if (this.inProgressDownloads.has(item.guid)) {
			this.debug(`[enqueueDownload] Item ${item.guid} already in progress, skipping`);
	...
	private async enqueueDocumentForGroupSync(item: Document): Promise<void> {
		// Skip if already in progress
		if (this.inProgressSyncs.has(item.guid)) {
			this.debug(`[enqueueDocumentForGroupSync] Item ${item.guid} already in progress, skipping`);
	...
	addToLog(entry: Partial<SyncLogEntry>): void {
		...
		const id = entry.id || `log-${entry.type}-${entry.guid}-${entry.status}-${timestamp}`;
	 ...
	 */
	subscribeToSyncLog(callback: Subscriber<ObservableSet<SyncLogEntry>>): Unsubscriber {
