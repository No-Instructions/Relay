...
export class CanvasPlugin extends HasLogging {
	 ...
	 * this file's (viewDataBelongsToFile) — checked at construction, for a
	 * view whose load ran before the plugin attached, and again whenever
	 * the machine settles into a reconcile while the view is still unowned.
	 */
	 ...
	 * view.file: the rendered node ids are a non-empty subset of the ids
	 * this file holds in its local CRDT or on disk. A view that still
	 * renders another file's nodes, or nothing while its load is in
	 * flight, waits for the next setViewData or native save instead.
	 *
	 * Runs at construction and again from every reconcile the machine
	 * emits while the view is unowned. A plugin that attaches after
	 * Obsidian's load has already run (a plugin reload with the canvas
	 * open, a reopened tab whose read finished first) never sees a
	 * setViewData for that load, so a single attempt is not enough.
	 * Concurrent requests coalesce into one read, with one more pass
	 * afterwards if a request arrived mid-flight.
	 */
	...
	private async checkViewDataOwnership(): Promise<void> {
		...
		try {
			...
			if (viewDataBelongsToFile(viewData, localData, diskData)) {
				this.debug(
					"view data ownership established from rendered node ids",
					file.path,
				);
				...
				this.debug(
					...
					file.path,
					{
						rendered: viewData.nodes.map((n) => n.id),
						local: localData.nodes.map((n) => n.id),
						disk: diskData.nodes.map((n) => n.id),
					},
				);
			}
		} catch (e) {
	 ...
	 * adding one) and leave through removeNode; both are patched in
	 * install, and the nodes present at install are hooked there.
	 */
	...
	private connectEmbedView(embedView: EmbedEditorView): void {
		...
		this.embedReleases.set(embedView, release);
	}
	...
	private install() {
		...
		this.unsubscribes.push(
			getPatcher().patch(this.canvas, {
				...
				addNode(old: (...args: unknown[]) => unknown) {
					return function (this: unknown, node: CanvasNode, ...rest: unknown[]) {
				...
				removeNode(old: (...args: unknown[]) => unknown) {
					return function (this: unknown, node: CanvasNode, ...rest: unknown[]) {
						try {
