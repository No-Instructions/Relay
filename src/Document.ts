	...
	implements IFile, HasMimeType, SyncParticipant, ServerUpdateSink
{
	...
	ensureRemoteDoc(): Y.Doc {
		...
		if (isNew) {
			...
			// Under read access remoteDoc is not seeded; the provider fills it.
			const seedUpdate =
				accessMode === "write"
	 ...
	 * An active machine is then sent the matching event; an idle machine
	 * asks getAccessMode when it needs the answer.
	 */
	...
	/**
	 * Replace remoteDoc with a fresh one for read access. With a provider
	 * integration the provider reconnects on the new doc; without one the
	 * machine is handed the new doc if it held the old one.
	 */
