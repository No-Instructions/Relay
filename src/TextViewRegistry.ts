...
export class TextViewRegistry extends HasLogging {
	...
	constructor(
		...
		private resolvesViewType: (viewType: string) => boolean,
	) {
		super();
	}
	 ...
	 * The returned unsubscriber removes the record — from the live set and
	 * from the settings file. Registration is idempotent per (plugin, type).
	 */
	register(
		pluginId: string,
		viewType: string,
		...
		for (const [pluginId, block] of Object.entries(current)) {
			...
			const kept = views.filter((row) => {
				...
				return (
					...
					this.resolvesViewType(type)
				);
			});
			if (kept.length === 0) continue;
			next[pluginId] = { ...block, views: kept };
		}
		return next;
	}
