 ...
 *
 * The result is frozen. There is one projected object per entry and every
 * consumer that reads that entry is handed the same one, so it belongs to all
 * of them and to none of them; freezing settles who it answers to. The views
 * hold their own state in private fields and are frozen themselves, and this is
 * that same decision one level down, on the values they hand out.
 */
