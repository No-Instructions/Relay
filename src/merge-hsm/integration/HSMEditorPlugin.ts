...
export class HSMEditorPluginValue implements PluginValue {
   ...
   * prove which EditorView that is) and whole-file canvas embeds. The
   * editorInfoField holds the owner object itself, fixed for the life of
   * the EditorState, so this classification keys on an immutable input and
   * is safe to be sticky. Everything the classification refuses is an
   ...
   * - Any editor whose owner is not a MarkdownView, except a whole-file
   *   canvas embed (identified by its iframe container). This refuses the
   ...
   *   binding. An embed scoped to a subpath is refused even inside the
   *   canvas iframe: its buffer is a fragment of the target note.
   */
  private probeSubEditor(): boolean {
    ...
    const insideIframe = !!this.editor.dom
      .closest(".markdown-source-view")
      ?.classList.contains("mod-inside-iframe");
    if (insideIframe && !fragmentScoped) return false;
    this.debug(
      fragmentScoped
