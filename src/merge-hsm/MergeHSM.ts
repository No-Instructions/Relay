...
import { DISK_ORIGIN, MACHINE_EDIT_ORIGIN, OpCapture, type CapturedOp } from "./undo";
import {
	isEmptyDoc,
	snapshotFromDoc,
	snapshotsEqual,
...
export class MergeHSM implements MachineHSM, SyncBridgeHost {
	...
	private _accumulatedEvents: Array<
		...
		| {
			...
			userEvent?: string;
			viewId?: string;
			machineEditAuthority?: MachineEditAuthority;
		}
	> = [];
	...
	private readonly _machineEditSessionId = uuidv4();
	...
	/** @internal Used by SyncBridge */
	getMachineEditOps(entry: PendingMachineEdit): CapturedOp[] {
		const opCapture = this.getOpCapture();
		if (!opCapture) return [];
		return entry.kind === "captured"
			? opCapture.byOrigin(entry.origin)
			: opCapture.sinceByOrigin(entry.captureMark, MACHINE_EDIT_ORIGIN);
	}
	...
	removeMachineEdit(entry: PendingMachineEdit): void {
		...
		if (idx >= 0) {
			...
			if (removed.kind === "captured") {
				...
				this._captureOpts?.trackedOrigins.delete(removed.origin);
				if (this._capturedMachineEditForkId === removed.id) {
	...
	async beginCapturedMachineEdit(
		...
		const origin = Symbol.for(
			`relay:machine-edit:${this._guid}:${this._machineEditSessionId}:${id}`,
		);
		this._captureOpts?.trackedOrigins.add(origin);
		...
		if (this._statePath === "active.tracking") {
			if (!this.localDoc) {
				this._captureOpts?.trackedOrigins.delete(origin);
				return null;
			}
		...
		if (!current || !this.localDoc || !this._fork) {
			if (!current) {
				this._captureOpts?.trackedOrigins.delete(origin);
				if (this._capturedMachineEditForkId === id) {
					this._capturedMachineEditForkId = null;
					if (this._fork?.origin === "machine-edit-captured") {
						this._fork = null;
					}
				}
			}
	...
	/**
	 * Open editors may receive Obsidian's rename repair directly through CM6,
	 * without a vault.process callback. Capture that exact before/after pair at
	 * the editor boundary so it enters the same isolated structural lane.
	 */
	private captureActiveMachineEditEvent(
		authority: MachineEditAuthority,
		output: string,
	): CapturedPendingMachineEdit | null {
		if (this._statePath !== "active.tracking" || !this.localDoc) return null;
		const input = this.localDoc.getText("contents").toString();
		const baseDoc = new Y.Doc({ gc: false });
		Y.applyUpdate(baseDoc, Y.encodeStateAsUpdate(this.localDoc));
		const capture = captureMachineEditSplices(baseDoc, input, output);
		baseDoc.destroy();
	...
	abortCapturedMachineEdit(handle: MachineEditCaptureHandle): void {
		const entry = this.findCapturedMachineEdit(handle.id);
		if (entry) {
			this._bridge.discardOutboundForMachineEdit(entry);
			if (
		...
		// A preparing capture gates inbound traffic. Every abort path, including
		// duplicate cleanup calls, must release that traffic immediately.
		this._bridge.flushInbound();
	}
		...
		return {
			...
			applyIdleMergeResult: (_hsm, event) => {
				...
				if (result.needsSync && this.localDoc) {
					this._bridge.syncFullStateToRemote();
				}
			},
			resetIdleRetryCount: () => {
			...
			updateLCAFromInvokeResult: (_hsm, event) => {
				...
				if (result?.newLCA) {
					...
					// WRITE_DISK is only an emitted request here. Disk metadata advances
					// when the executor directly confirms that the write finished.
					this.emitPersistState();
				}
			},
			...
			accumulateCM6Change: (_hsm, event) => {
				...
				this._accumulatedEvents.push({
					...
					viewId: e.viewId,
					machineEditAuthority: e.machineEditAuthority,
				});
			},
			...
			applyCM6ToLocalDoc: (_hsm, event) => {
				...
				if (this.localDoc) {
					let capturedMatch = this.matchCapturedMachineEditEvent(e);
					if (!capturedMatch && e.machineEditAuthority) {
						const createdEntry = this.captureActiveMachineEditEvent(
							e.machineEditAuthority,
							e.docText,
						);
						capturedMatch = this.matchCapturedMachineEditEvent(e);
						if (createdEntry && !capturedMatch) {
							this.removeMachineEdit(createdEntry);
						}
					}
	...
	private async invokeIdleRemoteAutoMerge(signal: AbortSignal): Promise<unknown> {
		...
		if (capturedMatch && this.remoteDoc) {
			const opCapture = this.getOpCapture();
			const machineOps = this.getMachineEditOps(capturedMatch);
			if (machineOps.length > 0) {
				if (opCapture?.canCancel(machineOps)) {
	...
	private async invokeForkReconcile(signal: AbortSignal): Promise<unknown> {
		...
		const currentRemoteStateVector = Y.encodeStateVector(remoteDoc);
		const remoteDroppedForkState = stateVectorIsAhead(
			fork.remoteStateVector,
			currentRemoteStateVector,
		);
		...
		if (mergeResult.success) {
			...
			if (
				...
				const alreadyApplied = resolveAppliedMachineEditSplices(
					localDoc,
					capturedForkEntry.capture,
				);
				if (alreadyApplied) {
					// A second fork can re-enter while the first deferred candidate is
					// still isolated. Preserve that origin and apply only the new merge
					// delta as ordinary local work; never fall through to a full publish.
					this.applyContentToLocalDoc(mergeResult.merged);
			...
			if (capturedForkEntry && remoteCarriesCapturedEdit) {
				...
				const ops = this.getMachineEditOps(capturedForkEntry);
	...
	private promoteCapturedMachineEditAtDeadline(
		...
		const opCapture = this.getOpCapture();
		const machineOps = this.getMachineEditOps(entry);
		...
		const reapply =
			...
			applied
				? this.previewCapturedPromotion(entry, opCapture, applied)
				: null;

		if (machineOps.length > 0 && reapply) {
			this._bridge.discardOutboundForMachineEdit(entry);
			this.withLocalObserverSuppressed(() => opCapture!.cancel(machineOps));
			const ytext = this.localDoc.getText("contents");
			this.localDoc.transact(() => {
			...
			// Published, inseparable, or drifted candidates cannot be destructively
			// cancelled. Release their tracking and publish them unchanged.
			opCapture?.drop(machineOps);
		}
	...
	/** Simulate cancel on a clone so an unresolvable candidate is never mutated. */
	private previewCapturedPromotion(
		entry: CapturedPendingMachineEdit,
		opCapture: OpCapture,
		applied: ResolvedMachineEditSplice[],
	): ResolvedMachineEditSplice[] | null {
		if (!this.localDoc || !entry.capture || !this._captureOpts) return null;
		const probe = new Y.Doc({ gc: false });
		let probeCapture: OpCapture | null = null;
		try {
			Y.applyUpdate(probe, Y.encodeStateAsUpdate(this.localDoc));
			probeCapture = OpCapture.restore(
				probe,
				probe.getText("contents"),
				opCapture.serialize(),
				this._captureOpts,
			);
			const probeOps = probeCapture.byOrigin(entry.origin);
			if (probeOps.length === 0) return null;
			probeCapture.cancel(probeOps);

			const resolved = resolveMachineEditSplices(probe, entry.capture);
			if (resolved) return resolved;
			const text = probe.getText("contents").toString();
			const fallback = applied.map((splice) => ({
				...splice,
				to: splice.from + splice.deleted.length,
			}));
			return fallback.every(
				(splice) => text.slice(splice.from, splice.to) === splice.deleted,
			)
				? fallback
				: null;
		} finally {
			probeCapture?.destroy();
			probe.destroy();
		}
	}
	...
	private matchCapturedMachineEditEvent(event: {
		...
		if (
			!this.localDoc ||
			(event.userEvent === "set" && !event.machineEditAuthority)
		) return null;
		...
		try {
			for (const { change, index } of ordered) {
				...
				try {
					...
					this._bridge.currentMachineEditId = entry.id;
					this.localDoc.transact(() => {
						Y.applyUpdate(this.localDoc!, update, entry.origin);
					}, entry.origin);
					machineApplied = true;
				} finally {
					this._bridge.currentMachineEditMark = null;
					this._bridge.currentMachineEditId = null;
					proxyDoc.destroy();
				}
			}
			this.syncFrontmatterToMap();
