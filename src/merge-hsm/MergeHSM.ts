...
/** Cheap synchronous signature identifying which text a repair replaced. */
...
export class MergeHSM implements MachineHSM, SyncBridgeHost, SyncMachine {
	...
	// Access mode for the current lock cycle; null outside one, where the
	// getAccessMode callback answers.
	private _activeAccessMode: ActiveAccessMode | null = null;
	private _getAccessMode: () => ActiveAccessMode;
	// Shared text last rendered while reading; an editor still holding it
	// has no reader-authored content to lose.
	...
	/** The lock cycle's mode inside one; the callback's answer outside. */
	...
	async registerMachineEdit(fn: (data: string) => string): Promise<void> {
		// Read mode registers no machine edit.
	...
	private buildGuards(): Record<string, GuardFn> {
		return {
			...
			// A discard rebuilds from the replica, so the replica must be synced.
		...
		return {
			...
			applyIdleMergeResult: (_hsm, event) => {
				...
				if (result.mergedContent !== undefined && result.needsDiskWrite !== false) {
					...
					if (this._disk !== null) {
						// Report the drifted content this write replaces.
	...
	private buildInvokeSources(): Record<string, InvokeSourceFn> {
		...
		return {
			...
			'idle-merge': async (_hsm, signal) => {
				...
				// Read access merges remote in and repairs disk from it, never forking.
				let result: unknown;
				if (readAuthority) {
	...
	private async invokeIdleRemoteAutoMerge(signal: AbortSignal): Promise<unknown> {
		...
		if (!this._lca && this._disk !== null) {
			...
			// Read access has no conflict surface, so park and leave disk alone.
			if (this.isReadMode()) {
				return { success: false, awaitingProvider: true };
			}
			this.pendingIdleUpdates = null;
	...
	private setupLocalDocObserver(): void {
		...
		this.localTextObserver = (event: Y.YTextEvent, tr: Y.Transaction) => {
			...
			// Dispatch only when the editor renders localDoc; a read fork renders the replica.
	...
	/**
	 * Text the editor shows in read mode: localDoc, or with a preserved fork
	 * the replica once its handshake has completed.
	 */
	...
	/** Report a Reader edit the shared version is about to replace. */
	...
	/** Drop write intent; the host replaced the replica before this event. */
	...
	/** Freeze localDoc as a fork whose base is the last shared ancestor. */
	...
	/**
	 * Judge local history against the synced replica: a fork the replica
	 * already holds is cleared; local-only ops without a fork get one.
	 */
	...
	/**
	 * Rebuild localDoc from the replica. The local store is cleared before
	 * the fork record is dropped, so an interruption leaves either the fork
	 * intact or an empty store the next load fills from the server.
	 */
