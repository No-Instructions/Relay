...
export const MACHINE: MachineDefinition = {
	...
	// replica a read state consults is server truth: the host replaces it
	// before the machine enters read mode, and read mode applies nothing
	// local to it. The audit on each provider sync compares local history
	// against it; write-era ops the server lacks are preserved as a fork that
	// freezes localDoc while the editor renders remoteDoc, until the user
	// explicitly resolves or discards it.
	// =========================================================================
	...
	'active.reading': {
		...
		on: {
			...
			// A discard issued while the replica is not server truth is dropped;
			// the fork stays and the user retries once connected.
