...
export interface TestHSM {
	...
	readonly stateHistory: Array<{
		from: StatePath;
		to: StatePath;
		event: MergeEvent["type"];
	}>;
...
export async function createTestHSM(
	options: TestHSMOptions = {},
): Promise<TestHSM> {
	const startTime = options.startTime ?? Date.now();
	const time =
		(options.timeProvider as MockTimeProvider) ?? new MockTimeProvider();
	...
	if (options.indexedDBUpdates) {
		updateRows.push({
			key: ++updateRowKeyCounter,
			update: options.indexedDBUpdates,
			origin: "seed",
		});
	}
	...
	const createPersistence = (
		...
		const initCapture = () => {
			...
			if (saved.length > 0) {
				persistence.opCapture = OpCapture.restore(
					doc,
					scope,
					{ entries: [] },
					captureOpts,
	...
	const hsm = new MergeHSM({
		...
		persistStateBarrier: async (state) => {
			effects.push({ type: "PERSIST_STATE", guid, state });
		},
		diskLoader,
