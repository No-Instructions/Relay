...
export async function createCrossVaultTest(
	options: CrossVaultTestOptions = {},
): Promise<CrossVaultTest> {
	...
	function sync() {
		for (let round = 0; round < 5; round++) {
			const hadWork =
				pendingFromA.length > 0 ||
				pendingFromB.length > 0 ||
	...
	function createVaultHandle(
		...
		return {
			...
			editText(newText: string) {
				...
				if (!localDoc)
					throw new Error("localDoc not available (HSM not in active mode)");
	...
	if (options.useProviderIntegration) {
		...
		const integrationB = new ProviderIntegration(
			hsmB.hsm as any,
			remoteDocB,
			providerB,
		);
		...
		syncFn = function providerSync() {
			...
			if (aConnected) {
				const updateForA = Y.encodeStateAsUpdate(
					server,
					Y.encodeStateVector(curRemoteDocA),
				);
			...
			if (bConnected) {
				const updateForB = Y.encodeStateAsUpdate(
					server,
					Y.encodeStateVector(curRemoteDocB),
				);
