 ...
 * during active entry.
 */
export async function sendAcquireLock(
	hsm: HSMHandle,
 ...
 * during active entry.
 */
export async function sendAcquireLockToTracking(
	hsm: HSMHandle,
	editorContent?: string,
): Promise<void> {
	if (editorContent !== undefined) {
	...
	await hsm.hsm?.awaitState?.(
		(s) => s === "active.tracking" || s.includes("conflict"),
	);
}
...
export async function loadToConflict(
	...
	await hsm.hsm?.awaitState?.(
		(s) => !s.includes("awaitingPersistence") && !s.includes("entering"),
	);
