...
class NetworkStatus {
	...
	private async probe(transport: Transport, primary = false): Promise<ProbeResult> {
		...
		const cancelled = new Promise<never>((_, reject) => {
			...
			timer = timeProvider.setTimeout(() => controller.abort(), PROBE_TIMEOUT_MS);
		});
		try {
			let request = this.requests.get(transport);
			if (!request) {
				request = this.request(transport, primary, controller.signal);
				this.requests.set(transport, request);
				const clear = () => {
					if (this.requests.get(transport) === request) this.requests.delete(transport);
				};
