...
class NetworkStatus {
	...
	private async _checkStatus(): Promise<void> {
		...
		for (let attempt = 0; attempt <= NETWORK_CHANGED_RETRY_LIMIT; attempt++) {
			let response: Awaited<ReturnType<typeof requestUrlWithMetrics>>;
			try {
				response = await requestUrlWithMetrics({
			...
			// Only the transport decides connectivity. A fault while applying
			// the response's messages is logged and leaves that answer alone.
			try {
				...
				this.log("Service message processing failed", error);
			}
