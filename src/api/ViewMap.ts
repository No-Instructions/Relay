 ...
 * subscription is inherited: the first delivery is immediate, and unsubscribing
 * after teardown is safe in either order because the base null-guards its
 * listener set and reaches the post office through `peekInstance()`. The one
 * thing not inherited is the first delivery once the post office is gone, which
 * the base has no way to make and a view still owes — see `subscribe` below.
 *
...
export class ViewMap<K, U, S> extends ObservableMap<K, U> {
	...
	override subscribe(run: Subscriber<ObservableMap<K, U>>): Unsubscriber {
		const unsubscribe = super.subscribe(run);
		if (PostOffice.isDestroyed()) {
			...
			// the entries frozen at teardown, and a subscriber that is never
			// called even once is left holding nothing at all rather than those
			// entries. The first delivery stays unconditional; only the updates
			// stop.
			run(this);
		}
		return unsubscribe;
