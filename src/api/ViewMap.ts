...
/**
 * Builds the copy of a stored value that a read hands back.
 *
 * The view keeps one projected value per entry and is read by many callers, so
 * a read returns `copy(stored)` rather than the stored value itself and each
 * caller gets an object of its own.
 */
export type Copy<U> = (value: U) => U;
 ...
 * at runtime rather than only to the compiler; and every read hands back a copy
 * of the stored value, so the contents are no one's to share either.
...
export class ViewMap<K, U, S> {
	...
	static create<K2, U2, S2>(
		...
		const view = new ViewMap<K2, U2, S2>(source, project, copy, observableName);
	...
	values(): U[] {
		return [...this.#map.values()].map((value) => this.#copy(value));
	}

	entries(): [K, U][] {
		return [...this.#map.entries()].map(
			([key, value]): [K, U] => [key, this.#copy(value)],
		);
	}
	...
	forEach(callbackfn: (value: U, key: K, map: Map<K, U>) => void): void {
		...
		// so iterating the backing store would pass it straight out. The pass runs
		// over a snapshot of copies instead, which is also what the callback's
		// value argument gets.
		const snapshot = new Map<K, U>();
		this.#map.forEach((value, key) => {
			snapshot.set(key, this.#copy(value));
		});
