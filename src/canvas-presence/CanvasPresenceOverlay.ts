...
import {
	...
	readOnlyCanvasPresence,
} from "./types";
import {
	type NativeCanvasPresenceAnchor as CanvasPresenceAnchor,
	type NativeCanvasPresenceEdgeEnd as CanvasPresenceEdgeEnd,
	type NativeCanvasPresenceGeometry as CanvasPresenceGeometry,
	type NativeCanvasPresenceItem as CanvasPresenceItem,
	type NativeCanvasPresenceState as CanvasPresenceState,
	type StateClass,
	fromCanvasPresence,
} from "./nativeState";
...
export interface CanvasPresenceOverlayOptions {
	...
	/** Role policy for the peer; awareness payloads do not grant editing permission. */
	canWriteContent: (user: CanvasPresenceUser | undefined) => boolean;
...
export class CanvasPresenceOverlay {
	...
	constructor(
		...
		this.surface = canvas.wrapperEl.createEl("canvas", { cls: "relay-canvas-presence" });
		...
		this.glyphBacking = win
			...
			: "#fff";
		const firstEdge = canvas.edges.values().next().value;
	...
	private renderClient(clientId: number, raw: unknown): boolean {
		...
		const user = readCanvasPresenceUser(raw);
		const canWriteContent = this.options.canWriteContent(user);
		const native = fromCanvasPresence(canWriteContent ? state : readOnlyCanvasPresence(state));
		const previous = this.peers.get(clientId);
		if (previous && (previous.canWriteContent !== canWriteContent ||
			(native.interactionId !== null && previous.state?.interactionId !== native.interactionId))) {
			// Permissions and gesture boundaries withdraw held previews and
			// reset interpolation before accepting a new interaction.
			this.removePeer(clientId, previous);
		}
		const peer = this.peers.get(clientId) ?? this.createPeer(clientId);
		peer.canWriteContent = canWriteContent;
		this.updatePeer(peer, clientId, native, user);
		return true;
	}
	...
	private renderItems(peer: Peer, state: CanvasPresenceState): void {
		...
		for (const [id, item] of Object.entries(state.items)) {
			const els = this.elementsFor(id, item.kind);
			if (els.length === 0) {
				wanted.delete(id);
				continue;
			}
	...
	private elementsFor(id: string, kind: "node" | "edge"): Element[] {
		const node = this.canvas.nodes.get(id);
		if (kind === "node") return node ? [node.nodeEl] : [];
	...
	private routeEdges(nodeId: string): void {
		...
		// Restore each node's own getBBox property as it was, or remove the
		// shadow so the prototype's method shows through again.
		const shadowed: Array<{ node: CanvasNode; own: PropertyDescriptor | undefined }> = [];
		for (const [id, bounds] of this.liveBounds) {
			const target = this.canvas.nodes.get(id);
			if (!target) continue;
			shadowed.push({ node: target, own: Object.getOwnPropertyDescriptor(target, "getBBox") });
		...
		try {
			...
			for (const entry of shadowed) {
				if (entry.own) Object.defineProperty(entry.node, "getBBox", entry.own);
