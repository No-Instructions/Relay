...
import {
	...
	type CanvasPresenceState as WireCanvasPresenceState,
} from "./types";
import {
	type NativeCanvasPresenceAnchor as CanvasPresenceAnchor,
	type NativeCanvasPresenceEdge as CanvasPresenceEdge,
	type NativeCanvasPresenceItem as CanvasPresenceItem,
	type NativeCanvasPresenceState as CanvasPresenceState,
	type NativeCanvasPresenceViewport as CanvasPresenceViewport,
	emptyNativeCanvasPresence as emptyCanvasPresence,
	readOnlyNativeCanvasPresence as readOnlyCanvasPresence,
	toCanvasPresence,
	interactionKey,
} from "./nativeState";
...
export type CanvasPresenceSink = (state: WireCanvasPresenceState) => void;
...
function normalizeItems(
	...
	for (const id of Object.keys(items).sort()) {
		...
		out[id] = geometry ? { kind: item.kind, classes, geometry } : { kind: item.kind, classes };
	}
	return out;
}
...
export class CanvasPresencePublisher {
	...
	private readonly canWriteContent: () => boolean;
	...
	private readonly gesturePrefix = uuidv4();
	...
	constructor(options: CanvasPresencePublisherOptions) {
		...
		this.canWriteContent = options.canWriteContent;
	...
	get current(): WireCanvasPresenceState {
		return toCanvasPresence(this.state);
	}
	...
	setCursor(
		...
		const rounded = this.canWriteContent() ? roundAnchor(anchor) : null;
		if (
	...
	clearCursor(): void {
		if (
			...
			!hasGeometry(this.state.items)
		) {
			return;
		}
		this.state = {
			...this.state,
			items: Object.fromEntries(Object.entries(this.state.items).map(([id, item]) => [id, {
				kind: item.kind, classes: item.classes.filter(name => name !== "is-dragging"),
			}])),
			mode: "attached",
			cursor: null,
	...
	setItems(items: Record<string, CanvasPresenceItem>): void {
		const normalized = normalizeItems(this.canWriteContent()
			? items
			: readOnlyCanvasPresence({ ...this.state, items }).items);
	...
	setEdge(edge: CanvasPresenceEdge | null): void {
		if (!this.canWriteContent()) edge = null;
		if (edgesEqual(this.state.edge, edge)) return;
		this.state = { ...this.state, edge: edge ? { ...edge, from: { ...edge.from }, to: { ...edge.to } } : null };
		this.schedule(this.cursorInterval);
	}

	/** Publish the current state again: the awareness field was reset. */
	republish(): void {
		// Clear queued editing state immediately on demotion, so a later
		// promotion cannot revive a drag captured without write permission.
		if (!this.canWriteContent()) {
			this.state = readOnlyCanvasPresence(this.state);
	...
	private schedule(minInterval: number): void {
		...
		const key = interactionKey(this.state);
		if (key !== this.gestureKey) {
			this.gestureKey = key;
			this.state.interactionId = key ? `${this.gesturePrefix}:${++this.gestureSequence}` : null;
		}
		this.dirty = true;
