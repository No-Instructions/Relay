...
export class CanvasPresenceInput {
	 ...
	 * dragging. Nodes and connection endpoints are sampled before the
	 * anchor offset, so each snapshot pairs geometry with its own offset.
	 */
	...
	sampleItems(): void {
		...
		for (const node of canvas.nodes.values()) {
			...
			const item: CanvasPresenceItem = { kind: "node", classes };
		...
		for (const edge of canvas.edges.values()) {
			...
			if (classes.length > 0) items[edge.id] = { kind: "edge", classes };
		}
