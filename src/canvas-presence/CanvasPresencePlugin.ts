...
export class CanvasPresencePlugin extends HasLogging {
	...
	private offPermissions: Array<() => void> = [];
	...
	constructor(
		...
		this.publisher = new CanvasPresencePublisher({
			...
			canWriteContent: () => this.relayCanvas.canPublishContent,
		});
		this.overlay = new CanvasPresenceOverlay(view.canvas, {
			timeProvider,
			resolveName: options.resolveName,
			canWriteContent: (user) => this.relayCanvas.sharedFolder.canUserWriteContent(user?.id),
		...
		// a chance to notice.
		this.offProviderState = relayCanvas.subscribe(this, () => {
			this.bindAwareness();
		...
		const folder = relayCanvas.sharedFolder;
		this.offPermissions = [
			folder.subscribeToPermissionChanges(refreshPermissions),
			folder.subscribeToPeerPermissionChanges(refreshPermissions),
		];
		this.bindAwareness();
		void trackPromise(
	...
	destroy(): void {
		...
		for (const off of this.offPermissions) off();
		this.offPermissions = [];
		this.offAwarenessUpdate?.();
		this.offAwarenessUpdate = null;
		const awareness = this.awareness;
