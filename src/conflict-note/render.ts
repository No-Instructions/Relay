...
// Each sign is the path data of an 11px stroked icon.
const PLUS = ["M5 12h14", "M12 5v14"];
const MINUS = ["M5 12h14"];
const BACK = ["M9 14 4 9l5-5", "M4 9h10.5a5.5 5.5 0 0 1 5.5 5.5a5.5 5.5 0 0 1-5.5 5.5H11"];
const signFor = (b: NoteBlock, side: Side) => (leavesAsWas(b, side) ? BACK : removes(b, side) ? MINUS : PLUS);

const SVG_NS = "http://www.w3.org/2000/svg";
function signEl(doc: Document, paths: string[]): SVGSVGElement {
	...
	class {
		...
		place(places: Place[]) {
			...
			for (const p of places) {
				...
				mark.append(signEl(this.view.dom.ownerDocument, signFor(p.b, near)));
				mark.dataset.side = near;
				pill.append(mark);
				if (far) {
					const f = el(this.view.dom.ownerDocument, "relay-conflict-pill-mark is-far");
					f.append(signEl(this.view.dom.ownerDocument, signFor(p.b, "theirs")));
