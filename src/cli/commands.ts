...
async function attachRemote(
	...
	const other = ctx.relayManager.remoteFolders.values().find((remote) =>
		remote.guid === folder.guid && remote.relayId !== relay.id,
	);
	if (other) {
		throw new CliError("remote_exists", `${folder.path} still exists on another Relay Server (${other.relayId}); remove that server copy before moving it`);
	}
...
const share: CliCommand = {
	...
	async run(params, ctx) {
		...
		if (existing?.remote) {
			throw new CliError(
				"already_shared",
				`${path} is already on ${existing.remote.relay.name}; relay:remote:remove deletes the old server copy before moving`,
			);
		}
		await ensureVaultFolder(ctx, path);
...
const remoteAdd: CliCommand = {
	...
	async run(params, ctx) {
		...
		if (folder.remote) {
			throw new CliError(
				"already_shared",
				`${folder.path} is already on ${folder.remote.relay.name}; relay:remote:remove deletes the old server copy before moving`,
			);
		}
		const relay = resolveRelay(ctx, required(params, "relay"));
...
const remoteRemove: CliCommand = {
	...
	description: "Delete the server copy for every member; local files and history stay",
	flags: FOLDER_FLAG,
	async run(params, ctx) {
		const folder = resolveSharedFolder(ctx, required(params, "folder"));
		...
		ctx.sharedFolders.notifyListeners();
		return {
			data: { path: folder.path, guid: folder.guid, previousRelay: relayName, serverCopyDeleted: true },
			text: `${folder.path} is tracked; its server copy on ${relayName} was deleted`,
		};
	},
};
