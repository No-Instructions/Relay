...
export async function downloadAttachment(
	...
	// appendBinary arrived in Obsidian 1.12.3. An older adapter gets the
	// chunks held and written once at the end, the footprint of the buffered
	// download this replaces.
	const adapter = vault.adapter as unknown as {
		appendBinary?: (normalizedPath: string, data: ArrayBuffer) => Promise<void>;
	};
	const canAppend = typeof adapter.appendBinary === "function";
	...
	try {
		...
		for (let offset = 0; offset < size;) {
			...
			if (canAppend) await adapter.appendBinary!(path, response.arrayBuffer);
			else held.push(response.arrayBuffer);
			offset = end + 1;
			progress(offset, size);
		}
		checkAttachmentAbort(signal);
		if (held.length > 0) await vault.adapter.writeBinary(path, concatBuffers(held));
