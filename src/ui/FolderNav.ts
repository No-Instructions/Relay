 ...
 * applies. Storage and file-type reasons win over the read-only reason. The
 * read-only reason marks a file with no membership record in a folder
 * where this user cannot manage files, so it exists only on this device.
 */
