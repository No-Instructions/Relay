	...
	onOpen() {
		...
		this.component = new SelfHostModalContent({
			...
			props: {
				...
				onConfirm: async (url?: string, providerId?: string, organizationId?: string) => {
					const relay = await this.relayManager.createSelfHostedRelay(url, providerId, organizationId);
					this.close();
