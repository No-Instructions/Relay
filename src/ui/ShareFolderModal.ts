	...
	onOpen() {
		...
		this.component = new ShareFolderModalContent({
			...
			props: {
				...
				onConfirm: async (folderPath: string, folderName: string, isPrivate: boolean, userIds: string[]) => {
					const result = await this.onConfirm(folderPath, folderName, isPrivate, userIds);
					this.close();
					return result;
				},
			},
