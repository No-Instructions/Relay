...
export class SharedFolder extends HasProvider {
	...
	private deferDownloadUntilDocumentAccepts(
		...
		const existing = this._downloadsDeferredByState.get(vpath);
		if (existing) {
			// Already waiting on this same machine; one watcher is enough.
			if (existing.hsm === hsm) return;
			// The document was rebuilt under this path, so the old machine will
			// never answer. Wait on the live one instead.
			existing.unsubscribe();
		}

		const guid = doc.guid;
		const unsubscribe = hsm.stateChanges.subscribe(() => {
			if (!hsm.acceptsRemoteEnrollment) return;
			// The notification runs inside the machine's own event handling.
			// Re-drive on a fresh microtask so the download never re-enters the
			// machine part-way through a transition.
			void Promise.resolve().then(() => {
	...
	private resumeDownloadDeferredByState(path: string): void {
		...
		this.startDeferredDownloadForGuid(record.guid, {
			evenIfDocumentLoaded: !this.existsSync(path),
		});
	}
