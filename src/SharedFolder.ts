	...
	private addLocalDocs(types?: SyncType[]): void {
		...
		syncTFiles.forEach((tfile) => {
			...
			if (this.folderHSM) {
				...
				// Two things have to hold for that to be a wait rather than
				// a refusal. The hydration the machine is waiting on has to
				// be able to arrive (canConfirmMembership). And the path has
				// to really be pending-upload-only: a path the map already
				// commits carries two identities, and neither of the
				// machine's verdicts for that pair re-enqueues an upload —
				// it either keeps the hold (below confirmed confidence) or
				// supersedes it through a rebind. Deferring such a path here
				// leaves the user's file with no live document at all.
				if (
