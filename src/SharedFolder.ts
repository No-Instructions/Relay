...
export class SharedFolder extends HasProvider {
	...
	public isSyncableTFile(tfile: TAbstractFile): boolean {
		...
		const isSupportedFileType =
			Document.checkExtension(tfile.path) || SyncFolder.checkPath(tfile.path);
	...
	uploadDoc(vpath: string, update = true): Document {
		...
		const guid: string | undefined =
			this.syncStore.get(vpath) || this.pendingUpload.get(vpath);
		if (!guid) {
	...
	createDoc(vpath: string, update = true): Document {
		...
		const guid = this.syncStore.get(vpath) || this.pendingUpload.get(vpath);
		if (!guid) {
	...
	renameFile(tfile: TAbstractFile, oldPath: string) {
		...
		if (!newVPath && !oldVPath) {
			...
			if (!newVPath) {
				...
				if (this.pendingUpload.has(oldVPath)) {
