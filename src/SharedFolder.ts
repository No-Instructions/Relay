...
export class SharedFolder extends HasProvider {
	...
	// The authorization the server last granted this device for the folder,
	// remembered in the folder's own store. It answers before roles or a
	// token have arrived, so a Reader's device stays read-only on a cold
	// start, offline included. Absent means writable: every device starts
	// that way, and only a read-only grant the server actually gave can
	// change it.
	...
	constructor(
		...
		trackPromise(`folder:whenSynced:${this.guid}`, this.whenSynced())
			.then(async () => {
				...
				if (this.destroyed) return;
				this._persistedAuthorization = await this.readPersistedAuthorization();
				if (this.destroyed) return;
	...
	public get canManageFiles(): boolean {
		...
		return this._canManageFilesAnswerCache ?? this.persistedAccessAllows;
	}
	...
	private refreshPermissionPolicy(): void {
		...
		const remembered = this._persistedAuthorization != null;
		const canWrite = writeAnswer ?? assumed;
		const canManage = manageAnswer ?? assumed;
		if (this._lastCanWriteContent === null && this._lastCanManageFiles === null) {
			this._lastCanWriteContent = canWrite;
			this._lastCanManageFiles = canManage;
			if (remembered && (canWrite !== assumed || canManage !== assumed)) {
	...
	public set remote(value: RemoteSharedFolder | undefined) {
		...
		this._lastCanWriteContent ??= this.canWriteContent;
		this._lastCanManageFiles ??= this.canManageFiles;
		this._remote = value;
	...
	/**
	 * Remember the server's answer as each token arrives. A changed answer
	 * while roles are still unknown re-derives every document's access,
	 * since they may have been answering from the old memo.
	 */
	protected onClientToken(clientToken: ClientToken): void {
		const authorization = clientToken.authorization;
		if (!authorization || authorization === this._persistedAuthorization) {
			return;
		}
		this._persistedAuthorization = authorization;
		try {
			void this._persistence.set("authorization", authorization);
	...
	/** What the remembered grant allows when nothing fresher is known. */
	...
	private cleanupExtraLocalFiles(
		...
		const sync = (file: TAbstractFile) => {
			...
			// Absence alone is not deletion: Readers intentionally retain
			// untracked files without upload holds. Only adopt a removal
			// witnessed in membership, and never trash a nonempty directory
			// recursively (it may contain local-only children).
			if (
				reader && (
