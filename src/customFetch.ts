...
async function nodeHttpsRequest(
	...
	if (config.body !== undefined && config.body !== null && !(config.body instanceof ArrayBuffer)) {
		throw new Error("Node HTTPS transport requires an ArrayBuffer body");
	}

	const body = config.body instanceof ArrayBuffer ? Buffer.from(config.body) : null;
	const requestHeaders: Record<string, string> = {};
	new Headers(config.headers).forEach((value, name) => {
		requestHeaders[name] = value;
	});
	if (body) requestHeaders["content-length"] = body.byteLength.toString();
