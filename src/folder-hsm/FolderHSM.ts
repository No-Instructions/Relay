...
export class FolderHSM {
	...
	> {
		...
		return {
			// The committed identity at this path supersedes the path's own
			...
			// does not have. That bar doubles as the precondition for the
			// rebind the adoption emits: confirmed confidence only ever
			// arrives with a live exchange, which is exactly when a rebind
			// onto the committed history can run.
			committedIdentitySupersedesHold: (row) => {
				...
				const held = this.config.holds.getHold(row.path);
				return held !== undefined && held !== committed;
			},
			// Plain adoption of a committed identity, with no unpublished
			// mint competing for the path: either no hold at all, or a hold
			// whose identity IS what the map committed (our own mint
			// replicated back). A hold naming anything else belongs to the
			// supersession rung above — adopting over it here would leave
			// the row on the committed identity while the hold went on
			// resolving every path lookup to the mint, and would leave the
			// hold with nothing that can ever release it.
