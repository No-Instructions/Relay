...
export class FolderHSM {
	...
	private runClassification(): void {
		...
		for (const entry of mapEntries) {
			...
			if (
				Array.from(this.context.rows.values()).some(
					(row) => row.guid === entry.guid,
				)
			) {
				continue;
			}
	...
	private routeFileDiscovered(
		...
		if (this.getMapEntry(path)) {
			// Discovery of a mapped path settles immediately (a download
			// whose completion was missed); unmapped bootstrap discoveries
			// wait for the next classification pass.
			this.scheduleClassifyRow(seeded);
		}
	}
	...
	> {
		return {
			...
			emitRetractUpload: (row) => {
				...
				// a committed identity superseded the unpublished mint.
				this.emit({
					type: "RETRACT_UPLOAD",
					path: row.path,
