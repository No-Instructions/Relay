...
export const CANVAS_MACHINE: CanvasMachineDefinition = {
	...
	"idle.loading": {
		invoke: {
			...
			onDone: [
				...
				// Diverged parks: a file the engine cannot read as a canvas
				// proves nothing. A parseable file never routes here — with
				// a baseline it ingests three-way, without one additively.
	...
	"idle.remoteAhead": {
		...
		on: {
			...
			// A failed write also drops the recorded disk meta: the folder
			// poll's stat comparison then re-fires DISK_CHANGED on its next
			// cycle, so a transient failure retries instead of parking as a
			// permanent conflict.
			FLUSH_FAILED: {
				target: "idle.diverged",
				actions: ["recordFlushFailure", "clearDiskMeta"],
			},
			LOCAL_DOC_CHANGED: {
				target: "idle.remoteAhead",
	...
	"idle.ingesting": {
		...
		on: {
			...
			// Same poll re-arm as idle.remoteAhead: a failed (or aborted)
			// ingest must not park behind a matching stat.
			FLUSH_FAILED: {
				target: "idle.diverged",
				actions: ["recordFlushFailure", "clearDiskMeta"],
			},
	...
	// The disk file cannot be reasoned about — it fails to parse as a
	// canvas or could not be read — or a write failed and its retry is
	// pending. Parseable divergence never parks here: evaluation resolves
	// it (three-way with a baseline, additive union without). Nothing is
	// written; remote updates keep converging in CRDT space (downloads
	// are allowed), and a view attach takes over via the view path.
	"idle.diverged": {
		capabilities: {
			canEmitEffects: true,
			canDownload: true,
